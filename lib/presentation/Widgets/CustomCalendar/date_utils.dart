...
class CalendarUtils {
  ...
  static bool isDayOnMonth( DateTime day, DateTime onMonth ){
    //List<DateTime> daysOnMonth = daysInMonth( onMonth );
    //return daysOnMonth.firstWhere((element) => isSameDay(day, element)) != null;
    List<DateTime> daysOnMonth;
    var first = firstDayOfMonth(onMonth);
    var last = CalendarUtils.lastDayOfMonth(onMonth);
    daysOnMonth = daysInRange(first, last).toList();

    for (var value in daysOnMonth) {
      if(isSameDay(day, value)){
        print("days on month: ${daysOnMonth.length}");
        print("===>value: $value");
        print("===>day: $day");
        print("===>onMonth: $onMonth");
        return true;
      }
    }
    return false;
