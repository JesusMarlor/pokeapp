...
class _CalendarState extends State<Calendar> {
  ...
  Color colorType( TypeDay typeDay ){
    if( typeDay == TypeDay.test ){
      return Colors.green;
    }else if( typeDay == TypeDay.event ){
      return Colors.orange;
    }else if( typeDay == TypeDay.holiday ){
      return Colors.red;
    }else{
      return Colors.white;
    }
  ...
  void agregarEventos(){
    ...
    widget.events.forEach((key, value) {
      if( _selectedDate != null && key != null && CalendarUtils.isDayOnMonth(key, _selectedDate) ){
        print("---->si es del mismo mes");
  ...
  Widget get nameAndIconRow {
    ...
    if (!widget.hideArrows) {
      leftArrow = IconButton(
        ...
        icon: Icon(Icons.chevron_left, color: widget.primaryColor != null ? Colors.white : Colors.black,),
      );
      rightArrow = IconButton(
        onPressed: isExpanded ? nextMonth : nextWeek,
        icon: Icon(Icons.chevron_right, color: widget.primaryColor != null ? Colors.white : Colors.black),
      );
    } else {
      leftArrow = Container();
      ...
      return Expanded(
        child: _selectedEvents != null && _selectedEvents.isNotEmpty
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            final String start = DateFormat('dd MMM, y EEE').format(event.startTime).toString();
            final String end =  DateFormat('dd MMM, y EEE').format(event.endTime).toString();
            return Container(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 5,),
                            Text(event.description, style: TextStyle( fontSize: 18, fontWeight : FontWeight.w800,),)
                          ],
                        ),
                      ),
                    ),
  ...
  Widget cardTipo( String title, TypeDay typeDay ){
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            Icon( Icons.circle, color : colorType( typeDay ), size: 14, ),
            SizedBox( width: 5 ),
            Text(title)
          ],
        ),
  ...
  Widget build(BuildContext context) {
    return Container(
      child: Column(
        ...
        children: <Widget>[
          Container(
            color: widget.primaryColor != null ? widget.primaryColor : Colors.transparent,
          ...
          Container(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                cardTipo( "Test", TypeDay.test ),
                cardTipo( "Event", TypeDay.event ),
                cardTipo( "Holiday", TypeDay.holiday ),
              ],
            ),
          ),
          expansionButtonRow,
  ...
  ///cuando le da lick a un dia, lista los eventos de ese dia
  void handleSelectedDateAndUserCallback(DateTime day) {
    ...
    setState(() {
      _selectedDate = day;
      selectedWeekDays = CalendarUtils.daysInRange(firstDayOfCurrentWeek, lastDayOfCurrentWeek).toList();
